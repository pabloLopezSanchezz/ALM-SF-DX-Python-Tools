    ...
    for status, filename in differences:
        ...
        if status.startswith('R'):
            ...
            if status == 'A':
                ...
                handleDeletion( folder, apiname, mapDiffs )
...
def handleDeletion(folder, apiname, mapDiffs):
    if not folder in mapDiffs:
        mapDiffs[ folder ] = set()
    mapDiffs[ folder ].add( apiname )
    ...
    if folder in [ 'aura', 'lwc' ]:
        ...
        if '/' in apiname:
            ...
            if os.path.isdir( srcPath ):
                copyTree( srcPath, f'{deltaFolder}/{pathFolder}' )
            else:
                makeDirs( f'{deltaFolder}/{folder}' )
                copyFile( f'{srcFolder}/{folder}/{apiname}', f'{deltaFolder}/{folder}/{apiname}' )
        else:
        ...
        if '/' in apiname:
            ...
            fileName                = apiname.split( '/' )[ 1 ]
            pathFolder              = f'{folder}/{objectTranslationName}'
            makeDirs( f'{deltaFolder}/{pathFolder}' )
            
            copyFile( f'{srcFolder}/{folder}/{apiname}', f'{deltaFolder}/{folder}/{apiname}' )
            
            mainObjectTranslationFile = f'{objectTranslationName}.objectTranslation-meta.xml'
            if fileName != mainObjectTranslationFile:
                print( f'{INFO_TAG} objectTranslation dependency - including main file {mainObjectTranslationFile} for child metadata {fileName}' )
                mainObjectTranslationPath = f'{srcFolder}/{pathFolder}/{mainObjectTranslationFile}'
                if os.path.exists( mainObjectTranslationPath ):
                    copyFile( mainObjectTranslationPath, f'{deltaFolder}/{pathFolder}/{mainObjectTranslationFile}' )
                else:
                    print( f'Warning : Main objectTranslation file not found: {mainObjectTranslationPath}' )
        else:
            makeDirs( f'{deltaFolder}/{folder}' )
            copyFile( f'{srcFolder}/{folder}/{apiname}', f'{deltaFolder}/{folder}/{apiname}' )
    else:
